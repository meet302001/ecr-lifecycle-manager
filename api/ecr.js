...
function getSession(req, res) {
  if (!SECRET) { send(res, 500, { error: "SERVER_SECRET not configured." }); return null; }
  ...
  catch { send(res, 401, { error: "Session expired. Please reconnect." }); return null; }
}
...
function sanitizeError(err) {
  ...
  if (err.name === "ExpiredTokenException") return "Credentials have expired. Please reconnect.";
  if (err.name === "AccessDeniedException") return "Access denied. Ensure the IAM user has the required ECR permissions.";
  if (err.name === "RepositoryNotFoundException") return "One or more repositories were not found in the specified region.";
...
module.exports = async function handler(req, res) {
  ...
  if (!SECRET) return send(res, 500, { error: "SESSION_SECRET not configured." });
  ...
  try {
    ...
    if (action === "dry-run-start") {
      ...
      results.forEach((r, i) => {
        ...
        else failed.push({ repo: repos[i], error: "Preview could not be started." });
      });
    ...
    if (action === "dry-run-results") {
      ...
      const output = results.map((r, i) => {
        if (r.status === "rejected") return { repo: repos[i], status: "FAILED", error: "Preview failed.", images: [] };
        return {
          repo: repos[i], status: r.value.status, summary: r.value.summary,
          images: (r.value.previewResults || []).filter((img) => img.action?.type === "EXPIRE")
    ...
    if (action === "apply") {
      ...
      return send(res, 200, {
        results: results.map((r, i) => ({
          repo: repos[i], status: r.status === "fulfilled" ? "success" : "failed",
    ...
    return send(res, 500, { error: `${err.name}: ${err.message}` });
  }
};
